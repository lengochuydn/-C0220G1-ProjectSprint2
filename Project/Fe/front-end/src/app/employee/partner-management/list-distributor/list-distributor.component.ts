...
export class ListDistributorComponent implements OnInit {
  ...
  promiseAccount: any;
  typeOfDistributorList: TypeOfDistributor[];
  createFormAddMore: FormGroup;
  addMoreForm = new Array<FormGroup>();
  ...
  ngOnInit(): void {
    ...
    this.createFormAddMore = this.fb.group({
      id: [''],
      name: [''],
      numberPhone: [''],
      email: [''],
      ...
      website: [''],
      img: [''],
      typeOfDistributor: this.fb.group({
        id: [''],
        name: ['']
      }),
      deleted: ['false']
    });
    this.distributorService.findAllTypeOfDis().subscribe(
      next => this.typeOfDistributorList = next,
      error => {
        this.typeOfDistributorList = [];
        console.log(error);
      });
  }
  ...
  loadImgAvatar2(target: any, i: number): void {
    ...
    if (target.files && target.files[0]) {
      const reader = new FileReader();
      reader.onload = (e: any) => {
        console.log(e.target.resuilt);
        $('#avatar2'+ i).attr('src', e.target.result);
      };
      reader.readAsDataURL(target.files[0]);
      this.imgFile = target.files[0];
      // $('#myAvatar').val(null);
...
// GET MESS VALIDATE ========================================================
    ...
    if (isFormField.hasError('pattern') && isFormField.touched) {
      switch (field) {
        ...
        case ('fax'):
          ...
          break;
        case ('website'):
          return 'Website phải đúng định dạng : http://abc.com hoặc https://abc.com';
          break;
        default:
          return '';
      }
  ...
  // tslint:disable-next-line:typedef
  createMoreDis(){
    console.log(this.createFormAddMore.value);
    if (this.createFormAddMore.valid) {
      this.distributorService.create(this.createFormAddMore.value).subscribe(
        next => {
            console.log(this.createFormAddMore);
            alert('Bạn đã thêm mới thành công');
        }
      );
    } else {
  ...
  // tslint:disable-next-line:typedef
  addMore(){
    this.addMoreForm.push(this.fb.group(
      {
        id: [''],
        name: ['', [Validators.required, Validators.pattern('^[a-zA-Z0-9\\_\\-\\sÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮ' +
          'ẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹế]+$')]],
  ...
  checkInvalidForm2() {
    let check = false;
    // tslint:disable-next-line:prefer-for-of
    for (let i = 0; i < this.addMoreForm.length; i++) {
      if (this.addMoreForm[i].invalid) {
        check = true;
      }
    }
    return check;
  ...
  create2() {
    for (let i = 0; i < this.addMoreForm.length; i++) {
      if (this.addMoreForm[i].valid) {
        this.distributorService.create(this.addMoreForm[i].value).subscribe(
          () => {
            this.getAll();
            this.addMoreForm[i].reset();
            this.addMoreForm.splice(i, 1);
          },
          error => console.log(error)
        );
      }
    }
    this.addMoreForm = new Array<FormGroup>();
  }
  ...
  onFileSelected(event: any): void {
    if (event.target.files && event.target.files[0]) {
      ...
      const task = this.afStorage.upload(`BrandLogo/${this.selectedImage.name}_${time}`, this.selectedImage);
      task
        .snapshotChanges()
        .pipe(
          finalize(() => {
